...
function normalizeIterations(iterations) {
  ...
  return Number.isFinite(rounds) && rounds > 0 ? rounds : KDF_ITERATIONS;
}
...
export async function deriveVaultKey(password, salt, iterations = KDF_ITERATIONS) {
  const rounds = normalizeIterations(iterations);
...
export async function encryptBlobForIdb(file, key) {
...
export async function encryptBackupObject(obj, passphrase) {
  if (!window.crypto?.subtle) {
    throw new Error('Web Crypto API není dostupné.');
  }
  ...
  const key = await deriveBackupKey(
    passphrase,
    salt,
    Number(obj.crypto?.iterations) || KDF_ITERATIONS
  );
...
export async function decryptBackupObject(obj, passphrase) {
  if (obj?.kind !== 'LifeHub encrypted backup') {
    throw new Error('Soubor není šifrovaná LifeHub záloha.');
  }
