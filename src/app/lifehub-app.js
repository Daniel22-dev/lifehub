...
export function bootLifeHub(){
    ...
    function toast(msg,type='good'){
      ...
      setTimeout(()=>{box.style.opacity='0';box.style.transform='translateY(10px)';},4200);
    ...
    function modalDialog({title='Potvrzení', message='', confirmText='OK', cancelText='Zrušit', danger=false, fields=[], validate=null}={}){
      return new Promise(resolve=>{
        ...
        const onKeydown = event => {
          if(event.key === 'Escape'){ event.preventDefault(); cleanup(null); return; }
        ...
        setTimeout(()=>{(Object.values(inputs)[0] || ok).focus();}, 0);
      });
    }
    function confirmDialog(message, {title='Potvrzení', confirmText='Pokračovat', cancelText='Zrušit', danger=false}={}){
      return modalDialog({title, message, confirmText, cancelText, danger}).then(Boolean);
    }
    function passwordDialog({title='Heslo', message='', repeat=false, minLength=1, confirmText='Pokračovat'}={}){
      const fields = [
        {name:'password', label:'Heslo', type:'password', autocomplete:'new-password', minLength, required:true, placeholder:minLength>1 ? `Minimálně ${minLength} znaků` : ''}
      ];
      if(repeat) fields.push({name:'passwordRepeat', label:'Zopakovat heslo', type:'password', autocomplete:'new-password', minLength, required:true});
      return modalDialog({
        title, message, fields, confirmText,
        validate: values => {
