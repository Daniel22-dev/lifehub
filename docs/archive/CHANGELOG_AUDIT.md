...
## 3.1.5 modular step 4

- Vytaženy obecné UI pomocné funkce do `src/core/ui.js`.
- `toast`, `download`, `confirmDialog` a `passwordDialog` už nejsou součástí hlavního aplikačního monolitu.
- Chování aplikace se nemění; krok pouze zmenšuje `src/app/lifehub-app.js` a připravuje další postupné dělení.
- Syntax kontrola rozšířena o nový modul `src/core/ui.js`.
